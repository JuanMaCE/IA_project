...
left_circle = []
angulo_base = 2 * math.pi / 7
angulo_var = angulo_base
for i in range(7):
    left_circle.append(angulo_var)
    angulo_var += angulo_base
...
def position_left_circle(count_lines, radius, finger_x, finger_y, radius_x, radius_y):
    hip = math.sqrt((finger_x - radius_x) ** 2 + (finger_y- radius_y) ** 2)
    if hip <= radius:
        print(left_circle)
...
with mp_hands.Hands(
    ...
    while cap.isOpened():
        ...
        generar_lineas(8, image, 1000, 500, radius)
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                x2 = int(index.x * w)
                y2 = int(index.y * h)
                position_left_circle(7, radius, x2, y2, 250, 500)
